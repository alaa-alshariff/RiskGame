...
/**
 * This class describes information stores a WarMap to be used in the game, it's
 * main use is to store the countries, continents and adjency list of
 * neighbouring countries
 * It also contains the functions for validating, saving and showing the game
 * map.
 */
public class WarMap {
    /**
    ...
    /**
     * A hashmap of the countries in the WarMap, this will take the form of
     * HashMap(CountryID, Country)
     */
    private HashMap<Integer, Country> d_countries;
    /**
     * A hashmap of the continents in the WarMap, this will take the form of
     * HashMap(ContinentID, Continent)
     */
    private HashMap<Integer, Continent> d_continents;
    /**
     * A hashmap of the adjency list of the WarMap, this will take the form of
     * HashMap(CountryID, List of Neighboring Country IDs)
     */
     ...
     */
    public WarMap() {
        this("Default Name", new HashMap<Integer, Country>(), new HashMap<Integer, Continent>(),
    ...
    public WarMap(String p_mapName) {
        this(p_mapName, new HashMap<Integer, Country>(), new HashMap<Integer, Continent>(),
                new HashMap<Integer, HashMap<Integer, Integer>>());
     ...
     *
     * @param p_mapName     is the map's name (generally the file name).
     * @param p_countries   is a hashmap of the countries in the map where the
     *                      hashmap contains keys with the countries ID and the
     *                      values are the corresponding countries.
     * @param p_continents  is a hashmap of the continents in the map where the
     *                      hashmap contains keys with the continent Id's and the
     *                      values are the corresponding continents.
     * @param p_adjencyList is a hashmap of the neighbouring countries in the map
     *                      where the hashmap contains keys with the country IDs and
     *                      the values are a list of the country Id's of
     *                      neighbouring countries.
     */
    public WarMap(String p_mapName, HashMap<Integer, Country> p_countries, HashMap<Integer, Continent> p_continents,
            HashMap<Integer, HashMap<Integer, Integer>> p_adjencyList) {
        d_mapName = p_mapName;
    ...
    /**
     * Function used for adding a continent to the HashMap containing the continents
     * of the map.
     *
    ...
    /**
     * Function used for adding a Country to the HashMap containing the Countries of
     * the map.
     *
    ...
    /**
     * Function used for adding a neighbour country, this will add a neighbour to
     * the country, as well as update the adjency list.
     *
    ...
    /**
     * Function used for removing a neighbour country, this will remove a neighbour
     * from the country, as well as update the adjency list.
     *
     * @param p_countryID   the country who will have a neighbour added.
     * @param p_neighbourID the neighbouring country to be added.
     */
     ...
     * @param p_isvisited a HashMap of containing booleans for if the country has
     *                    been visited.
     */
     ...
     * @param p_adjencylistcontinent a HashMap containing the adjency list of
     *                               continents.
     */
    // DFS recursive function for Continents
    private void dfsHelperContinents(Integer p_country, HashMap<Integer, Boolean> p_isvisited,
            HashMap<Integer, ArrayList<Integer>> p_adjencylistcontinent) {
        p_isvisited.put(p_country, true);
     ...
     *
     * @return Returns a boolean which is true if the map is valid, and false if the
     *         map is invalid.
     */
                ...
                if (d_countries.get(l_countryid).getContinentID() == l_continent_Id) {
                    ...
                    for (Integer l_neighborcountryId : l_countryset.getValue().getNeighbouringCountries().keySet()) {
                        if (l_neighborcountryId != null
                                && d_countries.get(l_neighborcountryId).getContinentID() == l_continent_Id)
        ...
        if (validateMap()) {
            try {
                ...
                for (Entry<Integer, Continent> l_continent : d_continents.entrySet()) {
                    l_info.write(
                            l_continent.getValue().get_continentName() + " " + l_continent.getValue().get_armyBonus());
                    l_info.newLine();
                }
                l_info.newLine();
                ...
                l_info.newLine();
                for (Entry<Integer, Country> l_country : d_countries.entrySet()) {
                    l_info.write(l_country.getKey() + " " + l_country.getValue().get_countryName() + " "
                            + l_country.getValue().getContinentID());
                    l_info.newLine();
                }
                l_info.newLine();
    ...
    public void showMap() { // Show map for only map
        for (Map.Entry<Integer, Country> l_entry : d_countries.entrySet()) { // All the countries with their
                                                                             // neighbouring countries
            System.out.println("Country with ID: " + l_entry.getValue().get_countryID() + " and name: "
                    + l_entry.getValue().get_countryName());
            ...
            for (Integer l_neighborcountryId : l_entry.getValue().getNeighbouringCountries().keySet()) {
                ...
                if (d_countries.get(l_neighborcountryId) != null) {
                    System.out.println(d_countries.get(l_neighborcountryId).get_countryName() + " with country ID: "
                            + d_countries.get(l_neighborcountryId).get_countryID());
                }
                if (d_countries.get(l_neighborcountryId) == null) {
        ...
        for (Map.Entry<Integer, Continent> l_entry : d_continents.entrySet()) { // All the continents with the country
                                                                                // ids belonging to them
            System.out.println("Continent with ID: " + l_entry.getValue().get_continentID() + " and name: "
                    + l_entry.getValue().get_continentName() + " with army bonus: "
                    + l_entry.getValue().get_armyBonus());
    ...
    /**
     * A function which prints the WarMap to the console in a readable format which
     * also shows ownership of the countries based on the list of players passed.
     *
    ...
    public void showMap(List<Player> p_players) { // Show map for with player ownership
        showMap();
        for (Player l_player : p_players) { // Shows what countries are owned by the players with the corresponding
                                            // number of armies
            System.out.println(l_player.get_playerName() + " owns the following countries:");
            for (Country l_c : l_player.get_playerCountries()) {
                System.out.println(l_c.get_countryName() + " with countryID " + l_c.get_countryID() + " with "
                        + l_c.get_numOfArmies() + " armies.");
    ...
    /**
     * A function for removing a full continent including its countries from a
     * WarMap
     *
     * @param p_continentId The id of the continent
     */
